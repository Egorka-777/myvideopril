...
namespace VideoBatch {
    public sealed class ViewsSearchPanel : UserControl {
        ...
        public ViewsSearchPanel(Preferences prefs, YouTubeBackend be) {
            ...
            var note = new Label {
                Text = "Сетка: отметьте зрителей слева — все стартуют параллельно (пауза 3–5 с между открытием профилей Dolphin).",
            ...
            var leftLayout = new TableLayoutPanel { Dock = DockStyle.Fill, RowCount = 4, ColumnCount = 1, BackColor = Theme.Card };
            ...
            var viewerTools = new FlowLayoutPanel { AutoSize = true, WrapContents = true, BackColor = Theme.Card, Margin = new Padding(0, 0, 0, 4) };
            viewerTools.Controls.Add(Theme.MakeButton("Все", ghost: true, action: () => SetAllViewers(true)));
            viewerTools.Controls.Add(Theme.MakeButton("Снять", ghost: true, action: () => SetAllViewers(false)));
            leftLayout.Controls.Add(viewerTools, 0, 2);
            ...
            leftLayout.Controls.Add(viewers, 0, 3);
        ...
        void SetAllViewers(bool on) {
            for (int i = 0; i < viewers.Items.Count; i++)
                viewers.SetItemChecked(i, on);
        }
        ...
        void LoadViewers() {
            ...
            foreach (var ch in settings.YouTubeChannels ?? new System.Collections.Generic.List<YouTubeChannel>())
                if (ch != null && NormMarket(ch.Market) == m && !string.IsNullOrWhiteSpace(ch.ProfileId))
        ...
        async Task RunMesh() {
            try {
                SaveSearchFields();
                string m = NormMarket(marketBox.SelectedItem?.ToString());
                settings.YouTubeMarketView = m;
                try { Store.Save(settings); } catch { }
                backend.SetMarketView(m);
                backend.Reload();
                var checkedChannels = new System.Collections.Generic.List<YouTubeChannel>();
                for (int i = 0; i < viewers.Items.Count; i++) {
                    if (!viewers.GetItemChecked(i)) continue;
                    if (viewers.Items[i] is YouTubeChannel ch) {
                        ch.Enabled = true;
                        checkedChannels.Add(ch);
                    }
                }
                if (checkedChannels.Count == 0) {
                    log.AppendText("Отметьте галочками аккаунты-зрители для сетки." + Environment.NewLine);
                    return;
                }
                int unique = checkedChannels.Select(c => (c.ProfileId ?? "").Trim().ToLowerInvariant()).Where(x => x.Length > 0).Distinct().Count();
                log.AppendText("Сетка: " + checkedChannels.Count + " отмечено" + (unique < checkedChannels.Count ? " · " + unique + " уникальных профилей" : "") + "." + Environment.NewLine);
